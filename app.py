...
def calculate_lufs(audio_path):
    ...
    try:
        ...
        logger.error(f"Error during LUFS calculation: {e}")
        import traceback
...
def create_lufs_graph(audio_path):
    ...
    try:
        ...
        for start in np.arange(0, len(data) / rate - interval, step):
            ...
            if len(segment) > 0:
                try:
                    ...
                    logger.warning(f"Error processing segment at {start}s: {segment_error}")
                    continue

        # Handle empty loudness_values
        if not loudness_values:
            logger.warning("No valid loudness values found")
            return {
                'time': [0],
        ...
        logger.error(f"Error creating LUFS graph: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return None
        ...
        try:
            ...
            if file and allowed_file(file.filename):
                ...
                if normalize and max_lufs is not None:
                    ...
                    logger.info(f"Will normalize to target LUFS: {target_lufs}")
                    ...
                    if noise_reduction:
                        ...
                        logger.info(f"Applying noise reduction with strength: {noise_strength}")
                    ...
                    if compression:
                        ...
                        logger.info(f"Applying compression: threshold={threshold}, ratio={ratio}, attack={attack}, release={release}")
                        ...
                        if processing_path != filepath:
                            try:
                                ...
                                logger.info(f"Removed temp file: {processing_path}")
                            except Exception as e:
                                logger.error(f"Failed to remove temp file: {e}")
                        
                        if compressed_path is None:
                            logger.error("Failed to apply compression")
                            return jsonify({'error': 'Failed to apply compression'})
                        
                        # Update processing path
                        processing_path = compressed_path
                        logger.info(f"Compression applied, new path: {processing_path}")
                    ...
                    logger.info("Normalization not requested, returning original data only")
                    return jsonify({
                        'lufs': original_lufs,
                        'original_graph_data': original_graph_data,
                        'is_stereo': is_stereo_audio
